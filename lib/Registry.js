...
class Registry {
    ...
    registerProvider({ name, host, port }) {
        let provider = this.providers.find(({ prvName, prvHost, prvPort }) => (prvName === name && prvHost === host && prvPort === port));
        if (!provider) {
            provider = {
                name,
                host,
    ...
    getProvider(name) {
        return this.providers.find((provider) => (provider.name === name && provider.r_expireAt > Date.now()));
    }
}
