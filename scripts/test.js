...
const client = net.createConnection({ port }, () => {
    ...
    invoke('user.getUserInfo', ['中午', 2, { name: 1, values: "tt" }]);
});
...
// testBuf.writeInt16LE(509338340, 0, 6);
// console.log(testBuf);
...
// 14: int16 - 4字节
// 15: int8 - 2字节
function invoke(serviceName, args) {
    const serviceId = getServiceId();

    console.log(serviceName);
    const buf = Buffer.alloc(8);
    buf.writeUInt32LE(serviceId, 0);
    buf.writeUInt32LE(serviceName.length, 4);

    const serviceNameBuf = Buffer.from(serviceName, 'utf8');

    if (args) {
        const argsBuffer = arrayToBuffer(args);
        client.write(Buffer.concat([buf, serviceNameBuf, argsBuffer]));
    } else {
        client.write(Buffer.concat([buf, serviceNameBuf]));
    }
